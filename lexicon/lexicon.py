LEXICON_RU = {
    ...
    'unexpected_message': 'Все говорят «{}», а ты бы лучше делом занялся!',
    'quest_last': 'Хмм... Постой, ответь, кто тебе по душе?'
}
...
_QUEST_LAST = dict(zip(
    QUEST_RES.keys(),
    'Тот самый дед,Бабушка в деревне,Француз на прогулке,'\
    'Обычный хлопец,Бабушка городская,Парень из соседнего подъезда,Первоклашка на продленке,'\
    'Самый главный начальник,Братишка-кавказец,Вальяжный кот-британец'.split(',')
))

BUTTONS_LEXIC.update(_QUEST_LAST)
